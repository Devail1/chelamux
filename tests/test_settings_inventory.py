...
def test_inventory_count_is_40():
    ...
    # critic group the same way — see the doc's "WIRED (CMX-220)" note). A change here
    # without a change to the doc's stated count is exactly the kind of drift this file
    # exists to catch — pinned as its own assertion so a future diff has to touch the
    # prose too.
    assert len(_scan_env_reads()) == 40
