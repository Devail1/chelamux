...
| **2** | **Attention auto-sort** | Needs-you panes sort to front + amber emphasis (the "sort" half of smart layout). | sort-order function (rank by `wantsHuman` > active > idle > done), stable within a rank. | needs-you tiles move to front on state change without reloading iframes. |
| **3** | **Presets + auto-arrange** | Curated presets (even-grid/focus-one/priority-split) + one-click auto-arrange/reset. | preset → `{wid:{x,y,w,h}}` layout map; auto-arrange is deterministic given a fleet. | each preset applies; reset snaps sane; persists to localStorage. |
| **4** | **Smart auto-layout (toggle)** | Status-driven sizing/position; builds on 2+3; user toggle, persisted. | layout-target function (status → cell/size), idempotent, respects the toggle-off manual layout. | toggling rearranges live; manual layout restored on toggle-off. |
