...
just the routing prefix — `assert cmd[4:] == ["-p", "MemoryMax", "-p", "MemoryCurrent"]`
for the `show` call, `assert "--type=slice" in cmd` for the `list-units` call — so a
request for the wrong property or unit type raises inside the fake itself instead of
silently returning fixture data shaped as if the request were correct.
