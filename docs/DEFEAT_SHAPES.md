...
**Recurred:** `chela/dispatcher.py`'s `_judge_watchdog` (CMX-282, PR #353, round 3) —
`test_an_expired_login_is_reaped_immediately_and_does_not_spend_a_retry` and its sibling
`test_an_expired_login_reaps_even_when_the_judge_lock_says_alive` both mounted
`patch.object(dispatcher, "_capture_pane", return_value=banner)`, so the stub answered the
login banner for *any* window argument. The judge changed
`_pane_shows_login_expired(_capture_pane(window))` to `_pane_shows_login_expired(_capture_pane(""))`
— reading whatever pane tmux happens to be pointing at instead of the judge's own window — and
both tests stayed green, because the flat `return_value` never distinguished the two calls.
Closed by replacing `return_value=banner` with a `side_effect` that only returns the banner
when called with the expected `window`, plus asserting `window in capture_calls`.
...
## 32. A ternary's mutually-exclusive branches share a keyword, so a substring assertion can't tell which branch rendered
