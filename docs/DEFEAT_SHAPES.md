...
**Also found:** CMX-288 rework round 3 (2026-08-14), PR #359 — `openDecisionsMenu`'s
`tickDecisions()` call. Its own doc comment claims two things happen on open: "marks every
currently-held event as seen" (`_markSeen()`) and the modal "is never showing a stale read"
(`tickDecisions()`). Round 2 closed a guard for the first claim (dead-coding `_markSeen()`
had zero coverage) but the second call, two lines below it in the same function, was left with
none — dead-coding it to `if (false) tickDecisions();` stayed green under the round-2 fix.
Closed by a behavioral test that counts `/api/log` requests before and after
`openDecisionsMenu()` and asserts a new one fired — reading network-call counts, not DOM
state, since `tickDecisions()`'s only externally-visible effect is the fetch it triggers.
...
## 32. A hand-authored fixture starts in the correct state by construction, so the suite never checks that the REAL artifact does too
...
## 33. Fixing one wiring point against the real artifact leaves its structurally-identical siblings uncovered
...
## 34. A wiring guard pins the caller's reference string but never the callee's binding
