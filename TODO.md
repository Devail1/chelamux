...
- [x] **Bridge commands: `/screenshot` + `/esc` + the Telegram command menu (port ccbot's slash-commands).** chela's inbound currently forwards EVERY `/command` straight to Claude Code via `send_tmux`, so bridge-level commands don't work and there's no "/" autocomplete. Add:
