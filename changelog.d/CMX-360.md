...
- **A merged PR whose run row was still `running` never reconciled.** A rework spawn
  ...
  rework loop now refuses to spawn onto a row whose PR already reads `merged`, and the
  merge-reconcile path now also closes an already-`running` row once its PR merges — killing
  its window — while leaving any `running` row whose PR is *not* merged completely alone.
  (CMX-360, #491)
