...
- **`chela update` no longer reports success while leaving services on old code.** Its
  ...
  `services_running_stale_code()` (the fact `doctor` already reports) and re-syncs and
  restarts exactly the services it names, on the same "must not live behind *we just
  pulled*" reasoning already applied to the plugin refresh three lines above it. Services
